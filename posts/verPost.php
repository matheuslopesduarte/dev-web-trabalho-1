...
if (isset($_GET['PageId']) && $_GET['PageId'] != null) {
        ...
        <script>
            $(document).ready(function () {
                ...
                $(document).on('click', '.delete-btn', function () {
                    var commentId = $(this).data('comment-id');
                    $.ajax({
                        url: '<?php echo $url . '/fx/comentarios.php' ?>',
                        type: 'post',
                ...
                $(document).on('click', '.like-btn, .dislike-btn', function () {
                    ...
                    $.ajax({
                        url: '<?php echo $url . '/fx/like_dislike.php' ?>',
                        type: 'post',
                ...
                function LikePagina() {
                    $.ajax({
                        url: '<?php echo $url . '/fx/likespag.php' ?>',
                        type: 'get',
                        data: { pagina_id: '<?php echo $Pageid ?>' },
                        success: function (response) {
                ...
                <div class="header-main-article">
                    ...
                    <div style="width: fit-content; margin: 0 auto">
                        ...
                        <img src='data:image/jpeg;base64,<?php echo base64_encode($row['cover_image']) ?>'
                            style="border-radius: 1vh; margin-top: 20px; width:100%">
                    </div>
                    <h1>
                        <?php echo $row['title'] ?>
                    </h1>
                    <p style="width: 90%; margin: 0 auto; padding: 20px 0; color: #555555">
    ...
    ';
                        ...
                        echo '<button class="jh-rb" id="login" href="' . $url . '/users/login.php?return=' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "
